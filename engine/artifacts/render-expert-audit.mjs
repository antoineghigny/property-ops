...
export function renderExpertAudit(caseData, analysis, profile) {
  ...
  report += `| **Execution Risk** | ${analysis.scores.execution_score}/100 | Technical Debt | ${analysis.scores.execution_score < 50 ? '🛑 High' : '✅ Manageable'} |\n\n`;
  ...
  if (redFlags.length > 0) {
    ...
    report += `- No critical deal-breakers identified, but technical and market risks remain (see below).\n`;
  }
  report += `\n---\n\n`;
  ...
  report += `This property is analyzed as a **${marketContextText}**. Combining an energy rating of **EPC ${peb}** with a **${Math.abs(round(pricing.relative_market_gap_pct, 1))}% market ${pricing.relative_market_gap_pct < 0 ? 'discount' : 'premium'}** requires a high-conviction strategic pivot.\n\n`;
  ...
  const taxes = economics.property_tax_annual_eur || (pricing.asking_price * 0.007);
  const insurance = 600;
